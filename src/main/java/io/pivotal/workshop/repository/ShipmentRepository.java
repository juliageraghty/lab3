...
public interface ShipmentRepository extends PagingAndSortingRepository<Shipment, Long> {
    ...
    @Query("SELECT NEW io.pivotal.workshop.domain.ShipmentJSON(s.account, s.shipped_date, s.delivery_date) from Shipment s WHERE s.account.id=:shipmentId ORDER BY s.shipped_date DESC")
